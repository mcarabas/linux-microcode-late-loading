...
%endif
%endif
%define rpmversion 4.13.%{base_sublevel}%{?stablerev}
...
%define gitrev 0
# Set rpm version accordingly
%define rpmversion 4.13.%{upstream_sublevel}
%endif
...
#%define kversion 4.1
%define kversion 4.13.%{base_sublevel}
...
# uek-playground disable dtrace support
%define with_dtrace 0
%endif
...
Patch1002: 0002-kbuild-Makefile.perf.patch
Patch1003: 0003-ml-rpm-Fix-netlink.h-compile-error-on-ol6.patch
...
%setup -q -n kernel-%{kversion} -c -T
      cp -rld $sharedir/kernel-%{kversion}/vanilla-%{kversion} .
    else
...
%if "%{kversion}" != "%{vanillaversion}"
  ...
  if [[ ! -z $sharedir  &&  -d $sharedir/kernel-%{kversion}/vanilla-%{vanillaversion} ]] ; then
    ...
    cp -rld $sharedir/kernel-%{kversion}/vanilla-%{vanillaversion} .

  else

    cp -rld vanilla-%{kversion} vanilla-%{vanillaversion}
...
cp -rld vanilla-%{vanillaversion} linux-%{kversion}-%{release}
...
## Apply custom uek-playground patches
## best effort patch for 0002-kbuild-Makefile.perf.patch
patch -p1 -F1 < $RPM_SOURCE_DIR/0002-kbuild-Makefile.perf.patch || \
echo "Warning: 0002-kbuild-Makefile.perf.patch patch FAILED."

## best effort patch for 0002-kbuild-Makefile.perf.patch
patch -p1 -F1 < $RPM_SOURCE_DIR/0003-ml-rpm-Fix-netlink.h-compile-error-on-ol6.patch || \
echo "Warning: 0003-ml-rpm-Fix-netlink.h-compile-error-on-ol6.patch patch FAILED."
...
cp %{SOURCE203} certs/ol_signing_keys.pem
...
/sbin/new-kernel-pkg --package kernel%{?-v:-%{-v*}} --install --make-default %{KVERREL}%{!-u:%{?-v:.%{-v*}}} || exit $?\
%{nil}
...
%ifarch x86_64
%kernel_variant_post -u -r (kernel%{variant}|kernel%{variant}-debug|kernel-ovs)
%else
%kernel_variant_post -u -r (kernel%{variant}|kernel%{variant}-debug|kernel-ovs)
%endif
